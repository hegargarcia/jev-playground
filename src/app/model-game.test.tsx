...
test("boards handle concurrent replies and resets without affecting another game", async () => {
    ...
    return new Promise<Response>((resolve) => {
      requests.push({
        ...
        complete: (move) => resolve(Response.json({
          move,
          confidence: null,
          options: board.flatMap((mark, square) => mark === null ? [{ square, weight: null }] : []),
        })),
      });
    });
  };
  ...
  try {
    ...
    assert.match(square(1, 4).getAttribute("aria-label") ?? "", /: O$/);
    assert.match(square(0, 4).getAttribute("aria-label") ?? "", /: empty$/);
    ...
    assert.ok(reset);
    await act(async () => { reset.click(); square(2, 8).click(); });
    ...
    assert.match(square(0, 0).getAttribute("aria-label") ?? "", /: empty$/);
    assert.match(square(0, 4).getAttribute("aria-label") ?? "", /: empty$/);
    ...
    assert.match(square(1, 2).getAttribute("aria-label") ?? "", /: X$/);
    assert.match(square(2, 8).getAttribute("aria-label") ?? "", /: X$/);
